...
-- ============================================================================
-- 4. VERIFICATION (run manually in staging, not part of the transaction above)
-- ============================================================================
-- Expect: ERROR  "Hard deletes are forbidden on belarro_v4_customer..."
--
--   DELETE FROM belarro_v4_customer WHERE id = '<some-id>';
--
-- Expect: success (soft delete path the app uses)
--
--   UPDATE belarro_v4_customer SET deleted_at = now() WHERE id = '<some-id>';
--
-- Expect: success (audited emergency override)
--
--   BEGIN;
--   SET LOCAL belarro.allow_hard_delete = 'on';
--   DELETE FROM belarro_v4_customer WHERE id = '<some-id>';
--   ROLLBACK;  -- or COMMIT during a real erasure window
-- ============================================================================
